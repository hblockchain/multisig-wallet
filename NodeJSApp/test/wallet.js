...
contract('Wallet', (accounts) => {
	let wallet;
	beforeEach(async () => {
		wallet = await Wallet.new([accounts[0],accounts[1],accounts[2]],
			2);
	...
	it('should create transfer', async () => {
		const abc = await wallet.createTransfer(100,accounts[5], {from: accounts[0]});
	...
	it('Should not approve transfer if sender is not approved', async ()=> {
		await wallet.createTransfer(100, accounts[5], {from: accounts[0]});
		await expectRevert(wallet.approveTransfer(0, {from: accounts[6]}),'only approver allowed');
	});
	...
	// 	await wallet.createTransfer(100, accounts[6], {from: accounts[0]});
	// 	await wallet.approveTransfer(0, {from: accounts[0]});
	// 	await expectRevert(wallet.approveTransfer(0, {from: accounts[0]}), 'should not approve transfer twice');
	// });
